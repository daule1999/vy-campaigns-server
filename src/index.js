...
const { templateRepository, personRepository, campaignRepository, userRepository, permissionRepository } = require('./db/repositories');
...
const start = async () => {
    ...
    // Seed default permissions (safe to run multiple times)
    try {
        await permissionRepository.seedDefaults();
    } catch (error) {
        console.error('Warning: Could not seed permissions:', error.message);
    }

    // Check if superadmin exists, create if not
    try {
        const superadminCheck = await userRepository.findByUsername('superadmin');
        if (!superadminCheck) {
            console.log('Creating superadmin...');
            await userRepository.createSuperAdmin();
        }
    } catch (error) {
        console.error('Warning: Could not check/create superadmin:', error.message);
        console.log('You may need to manually run database migrations if this is a fresh deployment.');
    }
...
║    RBAC:       /api/rbac       (permissions, groups)              ║
